	...
	_gsScope._gsDefine("utils.Draggable", ["events.EventDispatcher","TweenLite"], function(EventDispatcher, TweenLite) {
		...
		var _tempVarsXY = {css:{}}, //speed optimization - we reuse the same vars object for x/y TweenLite.set() calls to minimize garbage collection tasks and improve performance.
			...
			_addScrollListener = function(e, callback) {
				...
				if (e.parentNode && e !== _doc.body && e !== _docElement) {
				...
				if (e.getBBox && _isSVG(e)) { //SVG elements must be handled in a special way because their origins are calculated from the parent SVG canvas origin
					if (!e._gsTransform) {
						...
						if (e._gsTransform.xOrigin === undefined) {
							console.log("Draggable requires at least GSAP 1.15.0");
						}
					}
					v = e.getBBox();
					a = _getSVGOffsets(e);
			...
			Draggable = function(target, vars) {
				...
				var type = (vars.type || (_isOldIE ? "top,left" : "x,y")).toLowerCase(),
					...
					enabled, scrollProxy, startMouseX, startMouseY, startElementX, startElementY, hasBounds, hasDragCallback, maxX, minX, maxY, minY, tempVars, cssVars, touch, touchID, rotationOrigin, dirty, old, snapX, snapY, isClicking, touchEventTarget, matrix, interrupted, clickTime, startScrollTop, startScrollLeft, maxScrollTop, maxScrollLeft,
					...
					render = function(suppressEvents) {
						if (self.autoScroll && self.isDragging) {
							...
							var e = target,
								parent = e.parentNode,
								r1, r2, top, bottom, left, right, current;
							while (parent) {
								...
								if (bottom && !top) {
									if (maxScrollTop == null) {
										maxScrollTop = _getMaxScroll(parent, "y");
									}
									current = parent.scrollTop;
									if (current + self.autoScroll < maxScrollTop) {
										parent.scrollTop = current + self.autoScroll;
									} else if (current !== maxScrollTop) {
										parent.scrollTop = maxScrollTop;
									}
								} else if (top && !bottom) {
									current = parent.scrollTop;
									if (current - self.autoScroll > 0) {
										parent.scrollTop = current - self.autoScroll;
									} else if (current !== 0) {
										parent.scrollTop = 0;
									}
...
//TODO: sense direction of dragging instead of only looking at overlaps. If I drag up past the top, it should go that way even if the bottom is overlapping too.
//TODO: add capability to scroll window, not just elements. Make sure it still works if there's a long <div> on a page that's a parent, but the user drags to the bottom of the window.
								e = parent;
								parent = e.parentNode;

								if (top === bottom || parent === _docElement) {
									break;
								}
							}
							*/
					...
					onPress = function(e) {
						...
						if (!enabled || self.isPressed || !e) { //just in case the browser dispatches a "touchstart" and "mousedown" (some browsers emulate mouse events when using touches)
							console.log("returning onPressed");
							return;
						}
					...
					_getMaxScroll = function(element, axis) {
						var dim = (axis === "x") ? "Width" : "Height",
							...
							client = "client" + dim,
							body = _doc.body;
						return (element === window || element === _doc || element === body) ? Math.max(_doc[scroll], body[scroll]) - (window["inner" + dim] || Math.max(_doc[client], body[client])) : element[scroll] - element["offset" + dim];
					},
					/*
					_exceedsScrollTop = function(e) {
						var r1 = e.getBoundingClientRect(),
							...
							if (bottom && !top) {
								//console.log("down");
								maxTop = _getMaxScroll(parent, "y");
								return function () {
									var current = parent.scrollTop;
									if (current + self.autoScroll < maxTop) {
										parent.scrollTop = current + self.autoScroll;
									} else if (current !== maxTop) {
						...
						if (!autoScrollLeft) {
							...
							else if (!exceeded && parent !== _docElement && parent.parentNode) {
								return _exceedsScrollArea(parent);
							}
						}
						return exceeded;
					},
					*/
					...
					updateScroll = function(e) {
						if (self.isDragging) {
							var deltaX = startScrollLeft - target.parentNode.scrollLeft,
								deltaY = startScrollTop - target.parentNode.scrollTop;
							if (deltaX || deltaY) {
								startMouseX += deltaX;
								startMouseY += deltaY;
				...
				this.disable = function(type) {
					...
					_removeScrollListener(self.target, updateScroll);
					enabled = false;
					if (ThrowPropsPlugin && type !== "soft") {
